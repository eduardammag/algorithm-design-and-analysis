...
==================================================================
Enunciado (resumido):
A seguir são apresentados cenários que exigem a escolha de um algoritmo
de ordenação. Dentre os algoritmos apresentados em aula, proponha um
adequado para cada cenário, buscando a maior eficiência de tempo e espaço.
Explique a sua escolha.

a) Uma estação meteorológica precisa manter continuamente ordenados os n
maiores valores de temperatura em um sistema com memória limitada.
Dados chegam em tempo real... é essencial que o algoritmo apresente
complexidade O(n log n) no pior cenário.

b) Uma empresa de logística precisa ordenar 10 milhões de códigos de
rastreio (cada código: 12 dígitos numéricos). O sistema tem memória
suficiente para processar todos de uma vez. Proponha algoritmo e justifique.
"""

# Resposta (a): STREAMING dos n maiores com memória limitada
"""
IDEIA (a):
- Situação típica de 'top-n' em fluxo: não interessa ordenar todos os dados,
  apenas manter os n maiores observados até agora.
- Estrutura ideal: heap mínimo (min-heap) de tamanho n.
  - Inserção do próximo valor x:
    - Se heap não cheio: push(x) -> O(log n)
    - Se heap cheio e x > heap[0]: pop()+push(x) -> O(log n)
    - Caso contrário, ignorar x -> O(1)
- Complexidade:
  - Para m valores recebidos: O(m log n) tempo total.
  - Se considerarmos máximo m = O(n) (ou quando avaliamos o custo entre
    n valores finais), o pior caso clássico para ordenar n itens é O(n log n).
  - Espaço: O(n) (memória limitada satisfeita).
- Razão prática: mantemos apenas n elementos em memória e temos
  operações rápidas por chegada, garantindo escalabilidade.

Implementação (exemplo de uso do min-heap):
"""
import heapq
...
class TopNStreaming:
    def __init__(self, n):
        self.n = n
        self.heap = []  # min-heap de Python (lista)

    def offer(self, x):
        """Processa novo valor x do stream."""
        if len(self.heap) < self.n:
            heapq.heappush(self.heap, x)       # O(log n) quando cheio progressivamente
        else:
            # Se x for maior que o menor dos top-n, substitui
            if x > self.heap[0]:
                heapq.heapreplace(self.heap, x)  # pop+push em O(log n)

    def get_top_sorted_desc(self):
        """Retorna os top-n em ordem decrescente (copia O(n log n) para ordenar)."""
        return sorted(self.heap, reverse=True)


# Resposta (b): ORDENAR 10 milhões de códigos de 12 dígitos
"""
IDEIA (b):
- Os códigos são strings numéricas de comprimento fixo (12 dígitos).
- Temos memória suficiente para processar tudo de uma vez: podemos usar
  um algoritmo linear (radix sort) estável baseado em dígitos.
- Radix sort (LSD) em base b ordena em O(m * d) onde:
    m = número de itens (10M),
    d = número de "dígitos" / passes.
  Se escolhermos base b = 10^k, então d = 12 / k passes.
- Ex.: base 10^4 (k = 4) => d = 3 passes. Cada passe faz counting sort
  O(m + b), normalmente O(m) se b << m.
- Comparação com quicksort/mergesort:
    - Quicksort/mergesort: O(m log m) comparativo (m log m com m grande).
    - Radix com d constante (12 ou 3) é O(m) e muito eficiente para chaves
      com comprimento fixo.
- Recomendação prática: usar RADIX SORT (LSD) com base adequada (ex: 10^4)
  ou usar biblioteca otimizada se existir (std::sort é robusto, mas O(m log m)).
"""

def radix_sort_fixed_length_numeric(strings, base_pow=4):
    """
    Radix sort (LSD) para strings numéricas de mesmo comprimento.
    base_pow: número de dígitos por passe (ex: 4 -> base = 10^4).
    Complexidade: O(m * d). Estável.
    """
    m = len(strings)
    if m <= 1:
        return list(strings)
    ...
    base = 10 ** base_pow
    passes = (length + base_pow - 1) // base_pow

    # Counting sort por cada bloco de 'base_pow' dígitos, do menos significativo ao mais
    arr = list(strings)
    for p in range(passes):
        # índice do bloco de dígitos (LSB -> p=0)
        buckets = [[] for _ in range(base)]
        for s in arr:
            # extrair bloco de dígitos p (como inteiro)
            start = max(0, length - (p+1)*base_pow)
            end = length - p*base_pow
            block = int(s[start:end])  # trecho do número
            buckets[block].append(s)
        # re-concatenar
